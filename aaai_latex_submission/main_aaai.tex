...
bands (95\% CI overlapping, confirmed under subject-level clustering) --
the a priori hypothesis fails -- while the same layer bands differ
8.6$\times$ on training-induced rendering-defect rate (CI-decisive):
layer position governs how much collateral damage LoRA training does to
composition, not how much identity-conditioning capacity it learns. We
report both findings, including the failed hypothesis, as evidence that
identity-match-only evaluation is a systematically optimistic proxy for
reference-conditioning quality that future work in this space should not
rely on alone.
\end{abstract}
