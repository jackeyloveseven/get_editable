...
| `gemini_judge_ho30_band_0-15_r8_step4000.json` | **cross-set replication (2026-07-16)** of the band ablation: same four step_4000 checkpoints, zero retraining, re-run on the OTHER held-out 30-subject set (holdout30, disjoint from mix2/mix3/repprobe30), 2 seeds, raw prompt_B, same judge instrument. This file: early arm. Headline outcome across the four files: **the defect gradient does NOT replicate** (mid clustered 36.7% [20,53] vs full-depth 46.7% [30,63], overlapping; interior ordering inverts) while the **clean-yes identity null DOES replicate** (all arms 18.3-25.0%, CIs overlap) | `section_trainlight_band_ablation.md` replication addendum, aaai `30_results_traininglight.tex` + `40_limits_conclusion.tex` (rewrite of 2026-07-16) |
| `gemini_judge_ho30_band_10-25_r8_step4000.json` | same replication run, mid arm (run_mix1/step_4000, the published ablation's reference checkpoint, here re-run WITHOUT its original anime_2d prefix — this is the protocol-matched mid: defect 20.0% [12,32] vs its prefix-favored published 13.3%) | same as above |
| `gemini_judge_ho30_band_14-29_r8_step4000.json` | same replication run, late arm (defect 8.3% [4,18] — best arm on holdout30, inverting its second-worst rep30 rank) | same as above |
| `gemini_judge_ho30_band_0-29_r4_step4000.json` | same replication run, full-depth arm (defect 25.0% [16,37] — numerically worst in both sets, but never CI-separated from mid on holdout30) | same as above |
| `gemini_judge_ho30_mid_oldimages_rejudge.json` | judge-drift decomposition: the OLD production-mid holdout30 pixels (base_step4000, anime_2d-prefixed generations) re-judged with today's instrument — defect 13.3%→10.0% (93% per-image agreement), clean-yes 21.7%→21.7% (zero drift). Isolates judge drift (small, −3.3pp) from the prompt-prefix effect (~10pp) in the fresh-mid-vs-anchor gap | `section_trainlight_band_ablation.md` replication addendum |
| `ho30_band_replication_summary.json` | analysis summary for the replication (per-arm Wilson + subject-clustered bootstrap 20k, verdict flags, judge-drift block); produced by `scripts/64_ho30_band_replication_report.py`, whose `--selftest` first reproduces the published rep30 clustered numbers exactly before touching fresh data | same as above |
| `ho30_band_replication_summary.txt` | driver orchestration log for the replication run (arm start/done timestamps, gen/judge exit status, the harness-kill + detached-relaunch event) | provenance only |
| `random_defect_audit_band_ablation.json` | **seeded random defect audit (2026-07-16)** replacing the cherry-picked 3-of-17 spot-check: seed=0 sampling (code embedded in the JSON, re-executed verbatim and diffed to verify), full-depth 10/17 flags + late 5/11 + mid 2/2 + 4 mid negative controls, each judged at full resolution by a Claude-family auditor (different family from the Gemini judge). Result: full-depth 3 CONFIRMED / 4 BORDERLINE / 3 FALSE-POSITIVE, late 1/1/3, mid 0/2/0, negative controls 0/4 missed. Raw judge flag rates overstate severe-defect rates by ~1/3-2/3 per arm; gradient direction survives under the CONFIRMED-only lens (mid=0). Known judge failure modes documented: identity-mismatch complaints absorbed into visible_defect, one outright hallucination | `section_trainlight_band_ablation.md` caveats, aaai `30_results_traininglight.tex` + `40_limits_conclusion.tex` (dual-lens reporting) |
| `random_defect_audit_contactsheet.png` (+`_2.png`) | human-facing contact sheets for the audit above (sheet 1: full-depth samples; sheet 2: late + mid flags + negative controls) — the artifact a human eyeballs to independently confirm the auditor's calls | audit provenance |
| `gemini_judge_cleanmid_rep30_band_10-25_r8_step4000.json` | **confound-free mid re-run (2026-07-16)**: run_mix2/step_4000 (recipe-verified twin of the ablation arms — mix3 manifest, only save_every differs), raw prompt_B, on the ORIGINAL rep30 set. Result: defect 30.0% [20,43] per-image / 53.3% [37,70] clustered — **disjoint from the published confounded mid's 6.7% [0,17] on the same 30 subjects**; clean-yes 20.0% (unchanged). The decisive evidence that the published mid advantage was a protocol artifact | aaai `30_results_traininglight.tex` (dagger note + audit paragraph), `section_trainlight_band_ablation.md` addendum |
| `gemini_judge_cleanmid_ho30_band_10-25_r8_step4000.json` | same confound-free mid on holdout30: defect 26.7% [17,39] / clustered 46.7% [30,63], clean-yes 23.3% — cross-set stable (vs rep30's 30.0%), unlike the confounded measurement; also the same-protocol checkpoint comparison against run_mix1 (20.0% ho30) showing the manifest effect is statistically inseparable from run variance | same as above |
| `cleanmid_summary.json` | analysis summary for the confound-free re-run (both sets, Wilson + clustered, machine-readable confound_verdict, attribution bounds); produced by `scripts/65_cleanmid_report.py` (selftests against published numbers before computing) | same as above |
| `cleanmid_summary.txt` | driver orchestration log for the confound-free re-run | provenance only |
